...
export default function ReelsUpload({ 
  ...
  const [reelData, setReelData] = useState<ReelData>({ title: '', description: '', category: '', tags: [], coverPhoto: null });
  ...
  const categories = [
    'Entertainment', 'Music', 'Dance', 'Comedy', 'Education', 'Technology', 
    'Lifestyle', 'Fashion', 'Food', 'Travel', 'Sports', 'Gaming', 'Art', 'DIY'
  ];
  ...
  const pickReel = async () => {
    try {
      ...
      if (!result.canceled && result.assets[0]) {
        ...
        // Reels are typically shorter, max 100MB
        const MAX_SIZE = 100 * 1024 * 1024; // 100MB
        if (file.size && file.size > MAX_SIZE) {
          Alert.alert('File Too Large', 'Reel files must be less than 100MB');
          return;
        }
      ...
      Alert.alert('Error', 'Failed to pick reel file');
    }
  };
  ...
  const pickCoverPhoto = async () => {
    try {
      ...
      if (!result.canceled && result.assets[0]) {
        setReelData(prev => ({ ...prev, coverPhoto: result.assets[0] }));
      }
    } catch (error) {
  ...
  const handleUpload = async () => {
    if (!selectedFile) {
      Alert.alert('No File Selected', 'Please select a reel file first');
      return;
    }
    ...
    <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
      ...
      <View style={styles.uploadArea}>
        <TouchableOpacity 
          ...
          disabled={uploading}
        >
          {selectedFile && reelData.coverPhoto && (
            <View style={styles.coverPhotoOverlay}>
              <Image 
                source={{ uri: reelData.coverPhoto.uri }} 
          ...
          <Text style={[styles.uploadText, selectedFile && styles.uploadTextSelected]}>
            {selectedFile ? 'Reel Selected' : 'Choose Reel'}
          </Text>
          <Text style={styles.uploadSubtext}>
            {selectedFile ? `MP4 • ${formatFileSize(selectedFile.size)} • Max 100MB` : 'MP4, MOV, AVI (Max 100MB)'}
          </Text>
        </TouchableOpacity>
