...
AI3_Receiver::AI3_Receiver() : running_(false) {
    // Constructor
}

AI3_Receiver::~AI3_Receiver() {
    stopAsyncProcessing();
}

void AI3_Receiver::startAsyncProcessing() {
    if (running_) return;
    running_ = true;
    processingThread_ = std::thread(&AI3_Receiver::processingThread, this);
}

void AI3_Receiver::stopAsyncProcessing() {
    if (!running_) return;
    running_ = false;
    queueCV_.notify_all();
...
void AI3_Receiver::processingThread() {
    while (running_) {
        ...
        if (!videoPath.empty()) {
            ...
            // Add processed data to output queue
            {
                ...
                processedDataQueue_.push("AI3 Processed: " + videoPath);
            }
        }
    }
}

bool AI3_Receiver::isDataAvailable() {
    std::lock_guard<std::mutex> lock(queueMutex_);
    return !processedDataQueue_.empty();
}

std::string AI3_Receiver::getNextVideoData() {
    std::lock_guard<std::mutex> lock(queueMutex_);
...
void AI3_Receiver::enqueueVideo(const std::string& videoPath) {
    {
        std::lock_guard<std::mutex> lock(queueMutex_);
