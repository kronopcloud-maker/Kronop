...
AI5_Receiver::AI5_Receiver() : running_(false) {
    // Constructor
}

AI5_Receiver::~AI5_Receiver() {
    stopAsyncProcessing();
}

void AI5_Receiver::startAsyncProcessing() {
    if (running_) return;
    running_ = true;
    processingThread_ = std::thread(&AI5_Receiver::processingThread, this);
}

void AI5_Receiver::stopAsyncProcessing() {
    if (!running_) return;
    running_ = false;
    queueCV_.notify_all();
...
void AI5_Receiver::processingThread() {
    while (running_) {
        ...
        if (!videoPath.empty()) {
            ...
            // Add processed data to output queue
            {
                ...
                processedDataQueue_.push("AI5 Processed: " + videoPath);
            }
        }
    }
}

bool AI5_Receiver::isDataAvailable() {
    std::lock_guard<std::mutex> lock(queueMutex_);
    return !processedDataQueue_.empty();
}

std::string AI5_Receiver::getNextVideoData() {
    std::lock_guard<std::mutex> lock(queueMutex_);
...
void AI5_Receiver::enqueueVideo(const std::string& videoPath) {
    {
        std::lock_guard<std::mutex> lock(queueMutex_);
